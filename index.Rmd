...
ggplot(excess_data_long %>% 
         mutate(alpha = if_else(day_sameyear_floored > date_last_stats, "nedostupná", "dostupná")) %>% 
  ...
  labs(title = "Úmrtí: Covid-19 a nárůst navíc (excess deaths)",
       subtitle = str_glue("Týdenní počty zemřelých: nárůst spojený s Covid-19 x jiné příčiny\n",
                           "0 = průměr let 2017-19, přepočtený na věkovou strukturu roku 2019\n",
                           "Data o celkové úmrtnosti mají zpoždění.\n"),
...
ggplot(excess_data_long2 %>% 
  ...
  theme(legend.position = c(.33, .81), legend.key.size = unit(.5, "cm"),
        legend.box = "horizontal") +
  annotate("text", as.Date("1970-05-10"), 2300, label = "Průměr 2017-19", 
...
ggplot(excess_data_long %>% 
  ...
  scale_fill_manual(values = c(umrti_excess = "darkred",
                    ...
                    labels = c("Ostatní",
                               "Covid-19",
                               "Úroveň let 2017-19"), name = "Příčiny rozdílu proti 2017-19") +
  scale_alpha_discrete(range = c(1, .7), guide = "legend", name = "Data o všech úmrtích za 2020") +
  guides(fill = guide_legend(reverse = T, order = 1),
         alpha = guide_legend()) +
  theme_ptrr("both") +
