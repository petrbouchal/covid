...
lbs_grey <- tribble(~vek_txt, ~day_sameyear_floored, ~min, ~txt,
               factor("celkem"), as.Date("1970-05-10"), 12.1, "rozpětí\n2011-19",
               )
lbs_blue <- tribble(~vek_txt, ~day_sameyear_floored, ~min, ~txt,
               factor("celkem"), as.Date("1970-02-15"), 11.7, "2020"
               )
```
...
ggplot(data = zmr_bands, mapping = aes(group = vek_txt)) +
  ...
  labs(x = "Týden v roce",
       subtitle = str_glue("Údaje do {last_date_fmt}. Modrá = 2020, šedá = rozpětí let 2011-2019"),
