...
nhs <- read_csv("https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/nakazeni-vyleceni-umrti-testy.csv")
...
```{r narust-plot-percap}
nhs_narust %>% 
  filter(max(pocet, na.rm = T) > 0) %>%
  mutate(okres = fct_reorder(okres, narust_perk, max, na.rm = T, .desc = T)) %>% 
  ggplot(aes(datum, narust_perk)) +
  geom_col(fill = "darkblue", size = 1) +
  ...
  ptrr::theme_ptrr("y", multiplot = T) +
  theme(axis.text.x = element_text(hjust = 0)) +
  labs(title = "Nárůst per 100k za posledních 7 dní")
```
...
```{r age-grid}
cases_base <- nhs_bd %>% 
  filter(datum > "2021-09-01", vek < 130) %>% 
...
```{r, include=TRUE}
nhsts <- tsibble(nhs |> filter(datum > "2020-03-01"), 
                 index = datum)
...
## Vaccines {.tabset .tabset-pills}

### Doses & age

```{r ockod-data}
url_ocko_demo <- "https://onemocneni-aktualne.mzcr.cz/api/v2/covid-19/ockovani-demografie.csv"
ocko_demo <- read_csv(url_ocko_demo)
```


```{r ocko-process}
ocko_demo_e <- ocko_demo %>% 
  mutate(vakcina_s = 
           recode(vakcina, 
                  VAXZEVRIA = "AZ",
                  Spikevax = "Moderna",
                  ...
                  Comirnaty = "Pfizer"),
         poradi_status = 
           case_when(vakcina_s == "J&J" & poradi_davky == 1 ~ "Dokončení",
                     vakcina_s != "J&J" & poradi_davky == 2 ~ "Dokončení",
                     poradi_davky == 1 ~ "Zahájení",
                     TRUE ~ "Posilovací") %>% 
           as_factor() %>% fct_relevel("Zahájení",
                                       "Dokončení",
                                       "Posilovací"),
         vek_g = case_when(
           str_detect(vekova_skupina, "^[1-2]") ~ "do 29",
           str_detect(vekova_skupina, "^[3-5]") ~ "30-59",
           TRUE ~ "60+"
         ) %>% as_factor() %>% fct_relevel("do 29", "30-59"))
```


```{r ocko-plot, fig.asp=.6, out.width="100%"}
ocko_demo_e %>% 
  filter(datum > (today() - 30)) %>% 
  count(datum, poradi_status, vek_g,
        wt = pocet_davek) %>% 
  ggplot(aes(datum, n, fill = vek_g)) +
  facet_wrap(~poradi_status) +
  geom_col(width = .8) +
  scale_fill_viridis_d(direction = -1, end = .6, name = "věk") +
...
labs(title = "Vakcíny podle věku a pořadí dávky",
     subtitle = "Posledních 30 dní",
     caption = "@petrbouchal z dat MZd ČR")
...
vek_2020 <- vek_slct %>% 
  filter(rok == 2020, !is.na(vek_txt)) %>% 
  rename(obyv_2020 = pocet_obyv) %>% 
  select(-rok)
...
gd_dlouhe %>% 
  filter(kraj_nazev == "Celá ČR",
         date > "2021-09-01", kategorie_cz != "Parky") %>% 
