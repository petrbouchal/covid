...
```{r excess-data-daily, eval=F}
excess_data_daily <- excess_data %>% 
  mutate(weekstart_2020 = as.Date(paste(rok, tyden, 1, sep="-"), "%Y-%U-%u") - days(6)) %>% 
  complete(weekstart_2020 = full_seq(weekstart_2020, period = 1)) %>% 
  mutate(across(starts_with("umrti"), forecast::na.interp)) %>% 
  mutate(umrti_abs2020 = if_else(weekstart_2020 > max(zmr$day_sameyear[zmr$rok %in% 2020:2021]),
...
excess_data_daily %>% 
  ...
  scale_fill_manual(values = c(`Ostatní` = "pink", 
                               `Covid-19` = "lightblue"),
...
excess_data_long2 <- excess_data %>% 
  pivot_longer(c(umrti_covid, umrti_abs2020_noncovid)) %>% 
  mutate(name = fct_relevel(name, "umrti_covid"),
         value = if_else(is.na(value), umrti_mean_lastyrs, value))

date_last_stats <- max(zmr$day_sameyear[zmr$rok >= 2020])

ggplot(excess_data_long2 %>% 
         mutate(weekstart_2020 = as.Date(paste(2020, tyden, 1, sep="-"), "%Y-%U-%u") - days(6),
                alpha = if_else(weekstart_2020 > date_last_stats, "nedostupná", "dostupná")) %>% 
       aes(weekstart_2020)) +
  geom_col(aes(y = value, fill = name, alpha = alpha)) +
  geom_line(aes(y = umrti_mean_lastyrs, colour = "Úroveň let 2017-19")) +
  scale_color_manual(values = "grey10", name = NULL, guide = "none") +
  scale_fill_manual(values = c(umrti_abs2020_noncovid = "grey",
                               umrti_covid = "blue"),
                    labels = c("Covid-19",
                               "Ostatní"), name = "Příčiny rozdílu oproti 2017-19") +
